def pretify(raw_text):
    ...
    excluded = ['as', 'a', 'an', 'to', 'is', 'are', 'was', 'were', 'will', 'would', 'could', 'and', 'or', 'if', 'he',
                'she', 'it', 'this', 'my', 'i', 'there', 'some', 'but', '\'d']
...
while True:
    ...
    raw_text = input('Text for analysis:\n')

    entered_text = pretify(raw_text)

    if len(entered_text) < 5:
        print('Please enter text with at least 5 words (excluding pronouns, auxiliary verbs, '
              'adverbs, prepositions and conjunctions)')

    else:

        print('Thank you for using our services! Here is data you were looking for:')
        break

beautiful_text = pretify(raw_text)

counted_words = count_words(beautiful_text)
print(f'Wards Quantity: {counted_words}')

returned_dictionary = return_dictionary(beautiful_text)
print(f'Text Dictionary: {returned_dictionary}')

returned_keywords = return_keywords(beautiful_text)
print(f'Keywords: {returned_keywords}')

returned_statistics = return_statistics(beautiful_text, counted_words)
print(f'Frequency: {returned_statistics}')
